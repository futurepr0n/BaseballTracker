...
/**
 * Component for displaying active handicappers for hitters and selecting from master list
 * 
 * @param {Array} activeHandicappers - Currently active handicappers for hitters
...
const HitterHandicapperList = ({ 
  ...
  const handleHandicapperSelect = (option) => {
    if (option && option.value) {
      onActivateHandicapper(option.value, 'hitter');
    }
  };
    ...
    <div className="handicapper-list-container">
      <div className="handicapper-list-header">
        ...
        <div className="handicapper-actions">
          ...
          <button
            ...
            onClick={() => onAddHandicapper('hitter')}
            title="Add a new handicapper for hitters"
          >
      ...
      {activeHandicappers.length === 0 ? (
        <div className="no-handicappers-message">
          <p>No handicappers active for hitters. Add handicappers using the dropdown or "Create New" button above.</p>
        </div>
      ) : (
        <>
          <div className="handicapper-chips">
            {displayedHandicappers.map(handicapper => (
              <div key={handicapper.id} className="handicapper-chip hitter-handicapper">
                <span className="handicapper-name">{handicapper.name}</span>
                <button
                  className="remove-handicapper-btn"
                  onClick={() => onRemoveHandicapper(handicapper.id, 'hitter')}
                  title={`Remove ${handicapper.name} from hitters`}
                >
                  ×
                </button>
              </div>
...
export default HitterHandicapperList;