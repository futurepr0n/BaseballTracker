...
/**
 * Component for displaying active handicappers for pitchers and selecting from master list
 * 
 * @param {Array} activeHandicappers - Currently active handicappers for pitchers
...
const PitcherHandicapperList = ({ 
  ...
  const handleHandicapperSelect = (option) => {
    if (option && option.value) {
      onActivateHandicapper(option.value, 'pitcher');
    }
  };
  
  return (
    <div className="handicapper-list-container pitcher-handicapper-list">
      <div className="handicapper-list-header">
        <h3>Pitcher Handicappers</h3>
        <div className="handicapper-actions">
          ...
          <button
            ...
            onClick={() => onAddHandicapper('pitcher')}
            title="Add a new handicapper for pitchers"
          >
      ...
      {activeHandicappers.length === 0 ? (
        <div className="no-handicappers-message">
          <p>No handicappers active for pitchers. Add handicappers using the dropdown or "Create New" button above.</p>
        </div>
      ) : (
        <>
          <div className="handicapper-chips">
            {displayedHandicappers.map(handicapper => (
              <div key={handicapper.id} className="handicapper-chip pitcher-handicapper">
                <span className="handicapper-name">{handicapper.name}</span>
                <button
                  className="remove-handicapper-btn"
                  onClick={() => onRemoveHandicapper(handicapper.id, 'pitcher')}
                  title={`Remove ${handicapper.name} from pitchers`}
                >
                  ×
                </button>
              </div>
...
export default PitcherHandicapperList;